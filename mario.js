...
formElem.onsubmit = function(event) {
    ...
    // After you enter a value and submit it, then it automatically resets.
    event.preventDefault();

    // QUIZ
    // what happens if we don't do this?
    // Then the first error message and red border around the input will stay and never change.
    clearError();

    // figure out the height the user typed
    heightStr = heightElem.value;

    // If nothing is entered return this error message
    if(heightStr == ""){
    ...
    // negative numbers and zero also diplay error
    if (isNaN(height)) {
